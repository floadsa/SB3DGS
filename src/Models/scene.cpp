...
void Scene::ConvertFromObj(const char* objfilename)
{
    ...
    objects.back().AddSubmesh(Material(_name));
    ...
    if(_name == word2)
    {
    objects.back().submeshes.back().material = Material(_name);
    current_material = 1;
    }
    }   
	...
	float f1;
	mtlline >> f1;
	objects.back().submeshes.back().material.shininess = f1;
	}
    if(mtlword1 == "Kd")
    {
	float f1, f2, f3;
	mtlline >> f1 >> f2 >> f3;
    objects.back().submeshes.back().material.diffuse = glm::vec3(f1,f2,f3);
    }
    if(mtlword1 == "Ks")
    {
	float f1, f2, f3;
	mtlline >> f1 >> f2 >> f3;
    objects.back().submeshes.back().material.specular = glm::vec3(f1,f2,f3);
    break;
    }
    }
	...
	if (index[0] < keys.size())
	{
		for (int j = 0; j < keys[index[0]].size(); j++)
		{
			...
			{
			objects.back().submeshes.back().AddIndex(keys[index[0]][j].z-minus);
			objects.back().AddIndex(keys[index[0]][j].z-minus);
			t++;
	...
	if(t==0)
	{
		...
		objects.back().submeshes.back().AddIndex(counter-minus);
		objects.back().AddIndex(counter-minus);
		counter++;
		objects.back().AddPoint(Point(vertices[index[0]].x, vertices[index[0]].y, vertices[index[0]].z, tvertices[index[1]].x, tvertices[index[1]].y, nvertices[index[2]].x, nvertices[index[2]].y, nvertices[index[2]].z));
	}
	}
	}
	}
